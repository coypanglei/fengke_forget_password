    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item style="margin-bottom:13px;"
          ...
          :wrapperCol="wrapperCol"
          label="所有人权证号">
          <a-input placeholder="请输入所有人权证号" v-decorator="['syrqzh', validatorRules.v2]" />
        </a-form-item>
        <a-form-item style="margin-bottom:13px;"
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
          label="与借款人关系">
          <a-input placeholder="请输入与借款人关系" v-decorator="['yjkrgx', validatorRules.v3]" />
        </a-form-item>
        <a-form-item style="margin-bottom:13px;"
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
  ...
  export default {
    ...
    methods: {
      ...
      validatePhone(rule, value, callback){
        if(!value || new RegExp(/^1[34578]\d{9}$/).test(value) || new RegExp(/^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$/).test(value)){
          callback();
        }else{
          callback("请输入正确的联系电话!");
        }
