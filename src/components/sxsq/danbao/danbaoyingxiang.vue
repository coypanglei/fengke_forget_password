            ...
            <div class="asideBox">
              <dl class="menu-child">
                <a-spin :spinning="yingzhe">
                  ...
                  >
                    <a-icon v-show="item.wan" type="check-circle" style="color: #ffffff;" />
                    {{item.name}}
                  </dd>
                </a-spin>
              </dl>
...
export default {
    ...
    return {
      ...
      smalllist: [
        //   基础证件
        {
          ...
          wan: false,
          fileList: [
            { name: '', thumbUrl: '', uid: '', zllx: '担保人正', zhuang: false, diasb: false },
            { name: '', thumbUrl: '', uid: '', zllx: '担保人反', zhuang: false, diasb: false }
          ]
        },
        {
          name: '配偶身份证',
          ...
          wan: false,
          fileList: [
            { name: '', thumbUrl: '', uid: '', zllx: '担保人配偶正', zhuang: false, diasb: false },
            { name: '', thumbUrl: '', uid: '', zllx: '担保人配偶反', zhuang: false, diasb: false }
          ]
        }
      ],
      xuhao: 0,
  ...
  methods: {
    ...
    _changesmall(e) {
      this.xuhao = e
      for (let i = 0; i < this.smalllist.length; i++) {
        console.log(this.smalllist[i].status)
        ...
        .then(res => {
          if (res.success == true) {
            ...
            if (res.result.length == 0) {
              console.log('进入这个判断')
              for (let a = 0; a < this.smalllist.length; a++) {
                this.smalllist[a].wan = false
                for (let b = 0; b < this.smalllist[a].fileList.length; b++) {
                  this.smalllist[a].fileList[b].thumbUrl = ''
                }
              }
              return false
            }
            ...
            for (let i = 0; i < getshu.length; i++) {
              for (let k = 0; k < shuzu.length; k++) {
                for (let j = 0; j < shuzu[k].fileList.length; j++) {
                  if (getshu[i].zllx == shuzu[k].fileList[j].zllx) {
                    shuzu[k].wan = true
                    let image = window._CONFIG['domianURL'] + '/' + getshu[i].zldz
                    shuzu[k].fileList[j].thumbUrl = image
                    shuzu[k].fileList[j].diasb = true
                    that.smalllist[k].fileList[j].zhuang = true

                    shuzu[k].fileList[j].uid = getshu[i].id
                  }
                }
              }
            }
