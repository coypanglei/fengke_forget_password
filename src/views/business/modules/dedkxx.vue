<template>
  ...
  <div style="padding: 10px 20px;
    ...
    <a-spin :spinning="spinning">
      <!-- 基本信息头部 -->
      <div class="jia_top">
        <a-button type="primary" icon="save" v-show="list[0].id !==''&&list[0].id !==null" @click="baocun">保存</a-button>
      </div>
      <div class="daikuan">
        ...
        <div class="daikuan_bod" v-for="(item,index) in list" :key="index">
          ...
          <span>
						<!-- 4 -->
						<a-date-picker v-show="item.zzrq != null && item.zzrq != ''" class="data smallRight"
                       style=" width: 150px;height: 30px;font-size:12px"
                       :value="moment( item.zzrq, dateFormat) "
    ...
    />
      ...
      />
          ...
          <span>
						<!-- 5 -->
						<a-date-picker v-show="item.zqhdrq != null && item.zqhdrq != ''" class="data smallRight"
                       style=" width: 150px;height: 30px;font-size:12px"
                       :value="moment( item.zqhdrq, dateFormat) "
                       :allowClear='allowClear'
                       :format="dateFormat" placeholder=""
    ...
    />
      ...
      />
          ...
          <span>
						<!-- 9 -->
            <a-input v-model="item.dzywpm"></a-input>
          </span>
          <span>
						<!-- 10 -->
            <a-input v-model="item.dzywjz"></a-input>
          </span>
          <span>
						<!-- 11 -->
          ...
          <span>
						<!-- 12 -->
            <a-input v-model="item.sfbldzydj"></a-input>
          </span>
          <span>
						<!-- 13 -->
...
export default {
  ...
  methods: {
		...
		onChange(date, dateString, index) {
			this.list[index].zzrq = dateString 
			console.log( 	this.list[index].zzrq)
		},
		// 修改到期日期
		
		onChangea(date, dateString, index) {
			this.list[index].zqhdrq = dateString 
			console.log( 	this.list[index].zqhdrq)
		},
    getchuju() {
      var that = this
      that.spinning = true
      ...
      this.spinning = true
      getAction('/business/dhDedkflDkxx/queryBydeid', obj).then(res => {
        console.log(res)
        if (res.success == true) {
    ...
    baocun() {
      ...
      putAction('/business/dhDedkflDkxx/edit', {data:shuzu}).then(res => {
        console.log(res)
        if (res.success == true) {
